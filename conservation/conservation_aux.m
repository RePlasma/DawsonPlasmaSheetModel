function [maxerr] = conservation_aux(nparts,dtfac)
...
dt = dtfac*dx/(abs(vth)); % time step
tdim = round(tmax/dt); % number of iterations
ndump = 10; % time steps between dumps
...
enekin = zeros(ndumps,1); % kinetic energy in time
enepot = zeros(ndumps,1); % potential energy in time
...
for n=2:tdim
    ...
    if (mod(n,ndump)==0)
        ...
        enekin(idump) = sum(vi.^2);
        enepot(idump) = sum((xi-xieq).^2);
        mom(idump) = sum(vi);
...
maxerr = max(abs((ene-ene(1))/ene(1)));
